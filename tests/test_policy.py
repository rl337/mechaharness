...
from mechaharness.policy import Policy, PolicyFacts, PolicyThreshold, decide
...
def test_deny_precedence_and_unknown_signal() -> None:
    policy = Policy(
        ...
        thresholds=[
            PolicyThreshold(signal_id="destructive", deny_below=0.3, allow_above=0.8, ask_below=0.8),
        ],
    )
    deny = decide(
        PolicyFacts(action="delete"),
...
def test_ask_human_band_and_approval_reuse() -> None:
    policy = Policy(
        ...
        thresholds=[
            PolicyThreshold(signal_id="destructive", allow_above=0.9, ask_below=0.9),
        ],
    )
    ask = decide(
        PolicyFacts(action="noop"),
    ...
    missing = decide(
        PolicyFacts(action="mutate", action_digest="sha256:abc"),
...
def test_decisions_jsonl_and_replay(tmp_path: Path) -> None:
    ...
    policy = Policy(
        version="gate-fixture-v1",
        thresholds=[PolicyThreshold(signal_id="destructive", allow_above=0.5)],
    )
    signals = [NoulSignal(id="destructive", p_true=0.52)]
    verdict = decide(PolicyFacts(action="x"), signals, policy)
    log.record(
        DecisionRecord(
            event_id="d17",
            run_id="r1",
    ...
    replayed = replay_verdict(facts=PolicyFacts(action="x"), signals=signals, policy=policy)
